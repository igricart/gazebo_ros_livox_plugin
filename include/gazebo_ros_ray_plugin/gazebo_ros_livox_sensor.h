...
#ifndef GAZEBO_ROS_LIVOX_SENSOR_HH
#define GAZEBO_ROS_LIVOX_SENSOR_HH
...
namespace gazebo
{
  class GazeboRosLivoxSensor : public SensorPlugin
  {
    /// \brief Constructor
    public: GazeboRosLivoxSensor();
  ...
  template<typename T>
  void GazeboRosLivoxSensor::AdvertiseOutput()
  {
    ros::AdvertiseOptions ao =
      ros::AdvertiseOptions::create<T>(this->topic_name_, 1,
        boost::bind(&GazeboRosLivoxSensor::LaserConnect, this),
        boost::bind(&GazeboRosLivoxSensor::LaserDisconnect, this),
        ros::VoidPtr(), nullptr);
    this->pub_ = this->rosnode_->advertise(ao);
  }

  template<typename T>
  void GazeboRosLivoxSensor::SetParams(T sensor)
  {
